<template>
  ...
  >
    <template #activator="{ on, attrs }">
      ...
      >
        <template v-if="reaction">
          ...
          />
          <span
            class="d-none d-lg-inline-block pb-"
    ...
    <div class="like-buttons">
      <v-tooltip :disabled="breakpoint.mdAndDown" top>
        <template #activator="{ on, attrs }">
          <v-img
      ...
      <v-tooltip :disabled="breakpoint.mdAndDown" top>
        <template #activator="{ on, attrs }">
          <v-img
            class="love-btn d-inline-block cursor-pointer ml-2"
            v-bind="attrs"
      ...
      <v-tooltip :disabled="breakpoint.mdAndDown" top>
        <template #activator="{ on, attrs }">
          <v-img
            class="clap-btn d-inline-block cursor-pointer ml-2"
            v-bind="attrs"
      ...
      <v-tooltip :disabled="breakpoint.mdAndDown" top>
        <template #activator="{ on, attrs }">
          <v-img
            class="idea-btn d-inline-block cursor-pointer ml-2"
            v-bind="attrs"
      ...
      <v-tooltip :disabled="breakpoint.mdAndDown" top>
        <template #activator="{ on, attrs }">
          <v-img
            class="curious-btn d-inline-block cursor-pointer ml-2"
            v-bind="attrs"
...
export default {
  ...
  methods: {
    isReaction() {
      if (this.like.length) {
        const findRection = this.like.find((_) => _.user_id === this.user.id);
        if (findRection) {
          this.getReaction(findRection.reaction);
        }
      }
    },
      ...
      try {
        if (this.loggedIn) {
          const { deleted } = await this.$axios.$get(
            `api/like-post/${this.postId}/${reaction}`
          );
          if (deleted) {
            this.reaction = null;
            this.$emit('delete', {
