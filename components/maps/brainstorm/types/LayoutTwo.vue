<template>
  <div>
    <Scrollbar id="steps-draggable" class="vertical" :suppress-scroll-y="true">
      <div class="zoom-container" :style="{ transform: `scale( ${zoom}` }">
        ...
        >
          <v-row v-for="(step, i) in steps" :key="i">
            <v-col cols="12" md="6" :lg="stepDialog ? '3' : '6'" sm="6">
              <v-dialog
                v-if="stepIndex === i"
                v-model="stepBoxOpen"
                width="640"
              ...
              <v-card
                outlined
                height="252"
                class="move-cursor mt-2"
                @click.stop="openStepBox(i)"
              >
                <v-card-title
                  class="justify-space-between cx-card__title px-2 white--text"
                  :style="{
                ...
                >
                  ...
                  >
                    <template #activator="{ on, attrs }">
                      <div class="d-inline" v-bind="attrs" v-on="on">
                        <div
                          class="cx-title text-line-clamp"
                          :class="breakpoint.md ? 'font-16' : ''"
                        >
                          {{ step.title }}
                        </div>
                      </div>
                  ...
                  <div v-else :class="breakpoint.md ? 'font-16' : 'cx-title'">
                    <p class="mb-0 text-line-clamp">{{ step.title }}</p>
                  </div>
                  <v-menu
                    v-if="
                  ...
                  >
                    ...
                    <v-list class="pa-0">
                      ...
                      <v-list-item
                        ...
                        ripple
                        @click.stop="
                          openCopyDialog(step, map.type, map.group_id)
                        "
                      >
                        <v-list-item-action>
                          <v-icon>
          ...
          <v-row v-if="loggedIn && user.id === map.user_id">
            <v-col
              cols="12"
              md="6"
              :lg="stepDialog ? '3' : '6'"
              class="add-step"
            >
              <v-card v-if="stepBoxOpen && stepIndex === -1" outlined>
                <validation-observer
                  ref="stepForm"
                ...
                >
                  <v-form @submit.prevent="handleSubmit(addStep)">
                    <v-card-text>
                      <p v-if="error" class="red--text caption mb-3">
                        {{ error }}
                      </p>
                      <b-text
                        v-model="newStep.title"
      ...
      >
        <v-card v-if="activeStep">
          ...
          <v-card-text class="pa-3">
            ...
            <div
              v-if="!isCreateAnswer"
              class="d-flex justify-space-between align-center mt-6 mb-2 ml-2"
            >
            ...
            <!-- <v-divider /> -->
            <!-- Reaction actions -->
            <div v-if="!isCreateAnswer" class="d-flex mt-4">
              <answer-reaction
  ...
  methods: {
    ...
    async deleteAnswer() {
      ...
      try {
        ...
        setTimeout(() => this.closeAnswerDialog(), 100);
      } catch (error) {
        this.deletingAnswer = false;
      }
    },
    ...
    scrollDownAnswer(length, isLoadReplies) {
      ...
      // const getCommentlisSrollPosition = commentList.scrollTop;
