      ...
      <template v-else>
        <v-card flat class="mb-6 py-2" rounded outlined>
          <v-row class="px-4 my-1 align-center">
            <v-col class="py-0" cols="12" sm="6" md="8"
              ><h4
                class="
                  font-weight-medium
                  subtitle-1
...
export default {
  ...
  methods: {
    ...
    // search followers
    searchFollowing: debounce(async function () {
      try {
        const { data } = await this.$axios.$get(
          `api/user/following/${this.username}?query=${this.query}&page=1&limit=16`
        );
        this.members = data;
      } catch (e) {}
    }, 500),
