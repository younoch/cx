<template>
  <v-container class="pt-0">
    <v-row class="d-flex justify-start sticky-top z-index-3 background">
      ...
      <v-col cols="12" sm="6" md="3" lg="3" class="d-flex align-center">
        <search-field placeholder="Search images" @search="searchImages" />
      </v-col>
    </v-row>
        ...
        <v-col cols="12" md="9">
          <v-row>
            <v-col
              ...
              sm="6"
            >
              <galleryCard :card-data="item" type="image" />
            </v-col>
          </v-row>
        </v-col>
        <v-col cols="12" md="3">
    ...
    async getImages() {
      ...
      try {
        const { data } = await this.$axios.$get('api/get-posts-media', {
          params: { type: 'images', limit: 500 },
        });
        this.loading = false;
        this.images = data;
      } catch (e) {}
    },
    searchImages: debounce(async function (query) {
      try {
        this.loading = true;

        const { data } = await this.$axios.$get('api/get-posts-media', {
          params: { type: 'images', limit: 500, q: query },
        });
        this.images = data;
        this.loading = false;
      } catch (e) {}
